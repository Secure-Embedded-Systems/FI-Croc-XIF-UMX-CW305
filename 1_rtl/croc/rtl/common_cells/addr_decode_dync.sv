...
module addr_decode_dync #(
  ...
  /// Static address map (xsim 2020.2 workaround)
  parameter rule_t [NoRules-1:0] StaticAddrMap = '0
) (
  /// Address to decode.
  input  addr_t               addr_i,
  ...
  // xsim 2020.2 workaround: extract struct fields into flat wire arrays using genvar
  // (xsim can't handle StaticAddrMap[i].field with runtime loop variable i)
  logic [NoRules-1:0] rule_match;
  idx_t [NoRules-1:0] rule_idx_flat;

  for (genvar gi = 0; gi < NoRules; gi++) begin : gen_rules
    localparam addr_t RS = StaticAddrMap[gi].start_addr;
    localparam addr_t RE = StaticAddrMap[gi].end_addr;

    assign rule_match[gi] = (!Napot && (addr_i >= RS) && ((addr_i < RE) || (RE == '0))) ||
                            ( Napot && ((RS & RE) == (addr_i & RE)));
    assign rule_idx_flat[gi] = idx_t'(StaticAddrMap[gi].idx);
  end
  ...
  always_comb begin
    ...
    // match the rules — highest index wins on collision
    for (int unsigned i = 0; i < NoRules; i++) begin
      if (rule_match[i]) begin
        matched_rules[i] = ~config_ongoing_i;
        dec_valid_o      = ~config_ongoing_i;
        dec_error_o      = 1'b0;
        idx_o            = config_ongoing_i ? default_idx_i : rule_idx_flat[i];
      end
    end
  end
