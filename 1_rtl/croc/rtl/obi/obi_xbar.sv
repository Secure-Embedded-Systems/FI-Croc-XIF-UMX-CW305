...
module obi_xbar #(
  ...
  /// Static address map (xsim 2020.2 workaround: packed struct arrays corrupt via inputs)
  parameter addr_map_rule_t [NumAddrRules-1:0] StaticAddrMap = '0
) (
  input  logic clk_i,
  input  logic rst_ni,
  ...
  for (genvar i = 0; i < NumSbrPorts; i++) begin : gen_demux
    addr_decode #(
      ...
      .StaticAddrMap ( StaticAddrMap                       )
    ) i_addr_decode (
      .addr_i          ( sbr_ports_req_i[i].a.addr ),
      .addr_map_i      ( StaticAddrMap              ),
